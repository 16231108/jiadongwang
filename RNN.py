...
tf.app.flags.DEFINE_integer("layer_num",1,"number of ltsm layer")
...
tf.app.flags.DEFINE_integer("max_len",10,"we will use  ten point to predict the values of 11th")
tf.app.flags.DEFINE_enum("model_state","train",['train',"predict"],"model_state")
tf.app.flags.DEFINE_boolean("debugging",False,"delete log or not")
tf.app.flags.DEFINE_float("lr",0.01,"learing_rate")
...
if __name__ =="__main__":
    ...
    tf.logging.info(
        "train dataset prepared. train_x shape: {};train_y shape: {}".format(train_x.shape,train_y.shape))

    test_x,test_y=generate_data(np.sin(np.linspace(test_start,test_end,FLAGS.test_samples_num,dtype=np.float32)))
    tf.logging.info(
        "test dataset prepared. test_x shape: {};test_y shape: {}".format(test_x.shape,test_y.shape))
    rnn_model=RNN()
    rnn_model.build_net()
    if FLAGS.debugging:
